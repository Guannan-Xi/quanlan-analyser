...
def _seed_qc_related_artifact(state_store, task_model, artifact_model, derivatives_root: Path) -> None:
    task_id = "task_qc_related"
    output_dir = derivatives_root / PROJECT_ID / task_id
...
def main() -> int:
    ...
    primary_task_id = task_ids["multitaper_psd_tfr"]
    ...
    expected_modules = sorted(MODULE_TABLES)
    ...
    if not metrics_rows or metrics_rows[0].get("source_table") != "multitaper_band_power.csv":
        raise AssertionError(f"report package metrics.csv did not use the multitaper primary table: {metrics_rows[:1]}")
