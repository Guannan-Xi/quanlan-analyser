...
async function readArtifactText(page, artifacts, labelPattern) {
  ...
  const response = await page.request.get(`${API_BASE}/artifacts/${encodeURIComponent(artifactId)}/download`, { timeout: 60000 });
...
try {
  ...
  const uploadedReadback = await page.request.get(`${API_BASE}/eeg/files/${encodeURIComponent(uploaded.id)}`, { timeout: 30000 }).then((res) => res.json());
  ...
  const backendTask = await waitForTaskCompleted(page, 240000).catch(async (error) => {
    ...
    if (taskResponseJson?.id) {
      const readback = await page.request.get(`${API_BASE}/tasks/${encodeURIComponent(taskResponseJson.id)}`, { timeout: 60000 }).then((res) => res.json()).catch((readError) => ({ readback_error: readError?.message || String(readError) }));
  ...
  const backendArtifacts = await page.request.get(`${API_BASE}/tasks/${encodeURIComponent(backendTask.id)}/artifacts`, { timeout: 60000 }).then((res) => res.json());
  ...
  const finalState = await page.evaluate(() => {
    ...
    const resultImages = Array.from(resultImageGrid?.querySelectorAll("img") || []).map((img) => ({
    ...
    return {
      ...
      task,
      resultText,
      resultImageGridVisible: Boolean(resultImageGrid),
      resultImages,
      eventText,
      stageText,
  ...
  const packageResponse = reportJson?.id
    ? await page.request.get(`${API_BASE}/reports/${encodeURIComponent(reportJson.id)}/package`, { timeout: 60000 })
    : null;
