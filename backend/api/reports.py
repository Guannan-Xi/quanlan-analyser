...
def create_report(payload: ReportCreate, current: AccountRead = Depends(account_service.require_current_account)) -> ReportRead:
    ...
    return report_service.create_report(owned_payload)


@router.get("/reports/{report_id}", response_model=ReportRead)
def get_report(report_id: str, current: AccountRead = Depends(account_service.require_current_account)) -> ReportRead:
    return report_service.get_report(report_id, requesting_user_id=_requesting_user_id(current))
