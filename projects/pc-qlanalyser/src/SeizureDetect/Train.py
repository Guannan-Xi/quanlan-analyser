...
def Epilepsy_method(raw):


    # 使用示例
    parquet_path = r'C:\Users\evian\Desktop\QL\analysis_UI\SeizureDetect\m010001_r_labeled_edited_dataset.parquet'
    model_save_path = r"C:\Users\evian\Desktop\QL\analysis_UI\SeizureDetect\Pkl\model_bandpower.pkl"
   ...
   # print("预测完成，布尔数组长度:", len(pred_bool_array))
    ...
    epoch_length = 5 * sample_rate
    epoch_bool_array = np.zeros(len(pred_bool_array) // epoch_length, dtype=bool)

    # 比较pred_bool_array每epoch_length个元素中false和true的个数，谁多就赋值给epoch_bool_array
