...
def _pyeeg_hjorth_mobility(epoch):
    """兼容当前环境中 pyeeg 空包缺少 hjorth 的情况，优先保持原 pyeeg 行为。"""
    if hasattr(pyeeg, "hjorth"):
        hj_params = pyeeg.hjorth(epoch.tolist() if hasattr(epoch, "tolist") else epoch)
        return hj_params[1]
...
def _pyeeg_pfd(epoch):
    """兼容当前环境中 pyeeg 空包缺少 pfd 的情况，使用 Petrosian FD 等价公式。"""
    if hasattr(pyeeg, "pfd"):
        return pyeeg.pfd(epoch.tolist() if hasattr(epoch, "tolist") else epoch)
    ...
    if diff.size < 2:
        ...
        # 统计一阶差分符号变化次数，对应 Petrosian fractal dimension 的 N_delta。
...
def extract_features_using_epochs(data_segment, fs):
    ...
    for i in range(num_epochs):
        ...
        feat['mobility'] = _pyeeg_hjorth_mobility(epoch)  # 使用mobility参数
...
class Thread_run_analysis_ML(Thread_run_analysis):
    ...
    def _log_model_load_context(self, model_path, scaler_path):
        """记录打包环境下模型路径解析信息，便于定位 _MEIPASS / datas 问题。"""
        for label, path in (("model", model_path), ("scaler", scaler_path)):
            try:
                ...
                exists = False
                size = None
                QLLogging.log.exception(f"Failed to inspect ML epilepsy {label} file: {path}")

            QLLogging.log.info(
                "ML epilepsy %s load path=%s exists=%s size=%s sys.frozen=%s sys._MEIPASS=%s module_file=%s"
                % (
                    label,
                    path,
                    exists,
    ...
    def _limit_packaged_model_threads(self):
        """仅在 PyInstaller 打包模式限制模型线程数，降低 exe 内 joblib/OpenMP 卡住风险。"""
        ...
        for attr_name in ("n_jobs", "nthread"):
            ...
            try:
                ...
                QLLogging.log.info(
                    f"Packaged ML epilepsy model thread limit: {attr_name} {old_value} -> 1"
                )
            except Exception:
                QLLogging.log.exception(f"Failed to set packaged ML epilepsy model {attr_name}=1")
    ...
    def run(self):
        try:
            ...
            try:
                ...
                self._log_model_load_context(model_path, scaler_path)
                ...
                self._limit_packaged_model_threads()
                ...
                QLLogging.log.exception(f"Error loading ML epilepsy model/scaler: {type(e).__name__}: {e}")
            ...
            try:
                QLLogging.log.exception(f"ML epilepsy analysis thread error: {error_summary}")
                QLLogging.log.error(f"ML epilepsy analysis traceback:\n{traceback_text}")
            except Exception:
                logging.error(traceback_text)
            self.signal.emit([-1, error_summary])  # 发送错误信号，UI 会结束“分析中”状态
            return  # 确保线程正常退出
