...
def process_ecg_data(raw, selected_channel, time_range, progress_callback=None):
    ...
    if time_range:
        ...
        # 计算起始和结束索引
        start_idx = int(((start_time - raw.info['meas_date'].replace(tzinfo=timezone.utc)).total_seconds()) * sampling_rate)
        end_idx = int(((end_time - raw.info['meas_date'].replace(tzinfo=timezone.utc)).total_seconds()) * sampling_rate)+sampling_rate
        # 添加边界检查
    ...
    for i, start in enumerate(range(0, total_length, segment_length)):
        ...
        # 调整峰值索引并添加到列表
        if len(segment_peaks['ECG_R_Peaks']) > 0:
            adjusted_peaks = segment_peaks['ECG_R_Peaks'] + start
...
class Ui_ECG_compute(QWidget):
    ...
    def save_plot_widget_offscreen(self, start_time, end_time, save_dir, filename, dpi, img_format):
        ...
        x_time_window = x_time[time_mask]
        ax.plot(x_time[time_mask], self.ecg_results['ecg_signal'][time_mask],
                color='red', linewidth=0.7, label="ECG Signal")

        # 绘制R峰标记线
        r_peaks_times = np.array(self.ecg_results['r_peaks_idx']) / self.sfreq
        for x in r_peaks_times:
            if start_time <= x <= end_time:
                ax.axvline(x=x, color='k', linestyle='--', linewidth=0.7, alpha=0.7)
    ...
    def save_data(self):
        ...
        try:
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                time_deltas = [timedelta(seconds=t) for t in self.results['r_peaks_times']]
                test_times = [(start_time + delta).strftime("%H:%M:%S") for delta in time_deltas]

                # 对齐数据长度
                max_len = max(len(test_times),max(len(self.results[col]) for col in columns_of_interest if col in self.results))

                # 填充数据
                for key in columns_of_interest:
                    if key in self.results:
                        curr_len = len(self.results[key])
                        if curr_len < max_len:
                            padding = np.full(max_len - curr_len, np.nan)
                            self.results[key] = np.append(self.results[key], padding)
                # 创建DataFrame
                results_df = pd.DataFrame({
                    col: self.results[col] for col in columns_of_interest if col in self.results
                })
                results_df = results_df.iloc[:-1]  # 去掉最后一行
                # 添加心跳编号
                results_df.insert(0, 'Epoch No.', range(len(results_df)))
                # 选择要保存的列
                results_df = pd.DataFrame({
                    'Epoch No.': range(len(test_times)),  # 第一列保持为Epoch No.
                    'test_time': test_times,  # 新增的时分秒时间列（第二列）
                    'heart_rate': self.results['heart_rate'],
                    'r_peaks_times': self.results['r_peaks_times'],
                    'rr_intervals': self.results['rr_intervals'],
                    'DeltaRR': self.results['DeltaRR']
                })
                columns_to_select = ['Epoch No.','test_time', 'heart_rate','r_peaks_times', 'rr_intervals', 'DeltaRR']
                results_df_subset = results_df[columns_to_select]
