...
def detect_seizures(classifications, data, sfreq, epoch_length=1.0, start_time_ts=None, min_no_seizure_epochs=3):
    ...
    start_time_ts : float
        起始时间戳
    min_no_seizure_epochs : int
        最小无癫痫epoch数

    Returns:
    --------
    tuple : (seizure_info, seizure_per_minute)
        seizure_info : list of dict
            包含每次癫痫发作的详细信息
        seizure_per_minute : dict
            每分钟的癫痫发作统计
    """
    try:
        ...
        for i, is_seizure in enumerate(classifications):
            epoch_start_time = start_time_ts + i * epoch_length
            epoch_dt = datetime.datetime.fromtimestamp(epoch_start_time)
            minute_key = epoch_dt.replace(second=0, microsecond=0)

            if is_seizure:
                # 更新每分钟统计
                if minute_key not in seizure_counts:
                    seizure_counts[minute_key] = 0
                seizure_counts[minute_key] += 1

                if current_seizure is None:
                    # 开始新的癫痫发作
                    current_seizure = {
                        'start_time': epoch_dt.strftime('%Y-%m-%d %H:%M:%S'),
                        'start_sample': i * samples_per_epoch,
                        'epochs': [i]
                    }
                else:
                    current_seizure['epochs'].append(i)
            else:
                if current_seizure is not None:
                    # 检查是否应该结束当前癫痫发作
                    if len(seizure_info) == 0 or i - current_seizure['epochs'][-1] >= min_no_seizure_epochs:
                        end_time = start_time_ts + (current_seizure['epochs'][-1] + 1) * epoch_length
                        current_seizure['end_time'] = datetime.datetime.fromtimestamp(end_time).strftime('%Y-%m-%d %H:%M:%S')
                        current_seizure['end_sample'] = (current_seizure['epochs'][-1] + 1) * samples_per_epoch
                        current_seizure['duration'] = len(current_seizure['epochs']) * epoch_length
                        
                        # 计算这次发作的最大幅度
                        seizure_data = data[current_seizure['start_sample']:current_seizure['end_sample']]
                        current_seizure['max_amplitude'] = float(np.max(np.abs(seizure_data)))
                        
                        seizure_info.append(current_seizure)
                        current_seizure = None
        ...
        if seizure_counts:
            sorted_minutes = sorted(seizure_counts.keys())
            for minute in sorted_minutes:
                minute_stats['Minute'].append((minute - sorted_minutes[0]).total_seconds() / 60)
                minute_stats['Seizure Count'].append(seizure_counts[minute])
                minute_stats['UTC Time'].append(minute.strftime('%Y-%m-%d %H:%M:%S'))

        return seizure_info, minute_stats

    except Exception as e:
        logging.error(f"Error in detect_seizures: {str(e)}", exc_info=True)
        # 返回空列表和空字典，而不是None
        return [], {
            'Minute': [],
            'Seizure Count': [],
