...
mne.utils.set_config('MNE_LOGGING_LEVEL', 'ERROR')
...
def find_edf_files(directory: str, device: str = 'DSI') -> List[str]:
    ...
    try:
        for root, _, files in os.walk(directory):
            for file in files:
                if device == 'DSI' and "repaired" and "labeled"in file and file.endswith('.edf'):
...
def detect_seizures(classifications: np.ndarray, data: np.ndarray, sfreq: float,
    ...
    try:
        ...
        for i, pred in enumerate(classifications):
            if pred == 1:
                if not in_seizure:
                ...
                if non_seizure_count >= min_no_seizure_epochs:
                    ...
                    max_value = np.max(np.abs(seizure_data_segment))
        ...
        logging.error(f"检测癫痫发作时出错: {str(e)}", exc_info=True)
        return [], {
            'Minute': [],
            'Seizure Count': [],
            'UTC Time': []
