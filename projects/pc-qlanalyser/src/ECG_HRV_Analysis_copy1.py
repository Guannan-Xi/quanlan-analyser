...
def _run_hrv_nonlinear_backend_worker(peaks, sampling_rate, backend):
    ...
    timeout = float(os.environ.get("AR_ANALYSER_HRV_NONLINEAR_WORKER_TIMEOUT", "600"))
    with tempfile.TemporaryDirectory(prefix="ql_hrv_worker_") as tmp_dir:
        ...
        if proc.returncode != 0:
            raise RuntimeError(
                f"HRV nonlinear worker backend={backend} exited with code {proc.returncode}. "
                f"stdout={proc.stdout[-1000:]!r}, stderr={proc.stderr[-1000:]!r}"
            )
        if not os.path.exists(output_path):
            raise RuntimeError(f"HRV nonlinear worker backend={backend} did not produce output.")
        with open(output_path, "rb") as file_obj:
            payload = pickle.load(file_obj)
        if not payload.get("ok"):
            raise RuntimeError(
                f"HRV nonlinear worker backend={backend} failed: "
                f"{payload.get('error')}; traceback={payload.get('traceback')}"
            )
...
def calculate_hrv_nonlinear(peaks, sampling_rate):
    ...
    fallback_reasons = []
    QLLogging.log.info(f"hrv_nonlinear backend chain: {backend_chain}")
    ...
    for backend in backend_chain:
        if backend in {"cuda_cpp", "cuda_chunked"}:
            try:
                ...
                QLLogging.log.warning(f"hrv_nonlinear {reason}; trying next backend")
            except Exception as exc:
                reason = f"{backend}: {exc!r}"
                fallback_reasons.append(reason)
                QLLogging.log.warning(f"hrv_nonlinear {backend} worker failed, trying next backend: {exc}")
            continue
