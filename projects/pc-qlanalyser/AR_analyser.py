...
def _is_hrv_nonlinear_worker(argv):
    return len(argv) >= 5 and argv[1] == "--hrv-nonlinear-worker"


def _hrv_nonlinear_worker_main(argv):
    """HRV nonlinear GPU worker：在主 GUI 初始化前运行，避免触发单实例窗口。"""

    import multiprocessing
    import pickle
    import traceback

    # PyInstaller 子进程先执行 freeze_support，防止 worker 递归启动 GUI。
    multiprocessing.freeze_support()
    os.environ.setdefault("MPLBACKEND", "Agg")
    os.environ.setdefault("QT_QPA_PLATFORM", "offscreen")

    input_path, output_path, backend = argv[2], argv[3], argv[4]
    payload = {"ok": False, "error": None, "traceback": None}
    try:
        import AR_neurokit2 as nk

        with open(input_path, "rb") as file_obj:
            request = pickle.load(file_obj)

        backend_kwargs = {}
        workspace_budget_mb = float(os.environ.get("AR_ANALYSER_HRV_NONLINEAR_WORKSPACE_MB", "2048"))
        if backend == "cuda_cpp":
            # worker 内不做 Rust fallback；外层 GUI 负责 cuda_cpp -> cuda_chunked -> rust 链路。
            backend_kwargs.update(
                {
                    "cuda_cpp_workspace_budget_mb": float(
                        os.environ.get("AR_ANALYSER_HRV_CUDA_CPP_WORKSPACE_MB", workspace_budget_mb)
                    ),
                    "cuda_cpp_fallback_to_rust": False,
                }
            )
        elif backend == "cuda_chunked":
            # chunked 失败也交回外层链路，避免在 GPU worker 里长时间跑 CPU fallback。
            backend_kwargs.update(
                {
                    "cuda_chunked_workspace_budget_mb": float(
                        os.environ.get("AR_ANALYSER_HRV_CUDA_CHUNKED_WORKSPACE_MB", workspace_budget_mb)
                    ),
                    "cuda_chunked_fallback_to_rust": False,
                }
            )
        ...
        payload = {"ok": True, "hrv_nonlinear": hrv_nonlinear}
...
if _is_hrv_nonlinear_worker(sys.argv):
    sys.exit(_hrv_nonlinear_worker_main(sys.argv))
