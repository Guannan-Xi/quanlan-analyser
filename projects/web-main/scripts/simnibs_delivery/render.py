...
def _metric_table(data: Mapping[str, Any]) -> str:
    ...
    for row in data["results"]["roi_metrics"]:
        values = "".join(f"<td>{_esc(row['values'].get(key, '—'))}</td>" for key in metric_ids)
...
def render_report(data: Mapping[str, Any], root: Path | None = None) -> str:
    ...
    checks = "".join(f'<tr><td>{_esc(item["name"])}</td><td><span class="status {item["status"]}">{_esc(item["status"])}</span></td><td>{_esc(item.get("evidence", "—"))}</td></tr>' for item in data["quality_control"]["checks"])
