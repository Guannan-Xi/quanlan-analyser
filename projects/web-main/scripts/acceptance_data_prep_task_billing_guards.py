...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="qlanalyser-task-guards-") as tmp:
        ...
        formal_source_path = root / "uploads" / "owner-b-formal.edf"
        formal_source_path.write_bytes(b"not a real edf; owner-b formal fixture")
        formal_sha256 = hashlib.sha256(formal_source_path.read_bytes()).hexdigest()
        ...
        state_store.upsert_item(
            "projects",
            project_model.ProjectRead(
                id="proj_formal_task_guard_owner_b",
                name="Owner B formal task guard project",
        ...
        state_store.upsert_item(
            "eeg_files",
            eeg_model.EEGFileRead(
                id="eeg_formal_task_guard_owner_b",
                project_id="proj_formal_task_guard_owner_b",
                original_filename="owner-b-formal.edf",
                stored_path=formal_source_path,
                detected_format="edf",
                owner_user_id="owner-b",
                created_by="owner-b",
                size_bytes=formal_source_path.stat().st_size,
                sha256=formal_sha256,
                upload_status="uploaded",
                upload_authorization_confirmed=True,
                upload_authorization_text="Owner B confirms this fixture is authorized for formal task guard acceptance.",
                upload_authorization_confirmed_at=datetime.now(timezone.utc),
        ...
        formal_owner_a_plan = data_preparation_service.save_plan(
            prep_model.DataPreparationPlanCreate(
                project_id="proj_formal_task_guard_owner_b",
                input_file_id="eeg_formal_task_guard_owner_b",
                owner_user_id="owner-a",
                created_by="owner-a",
                status="confirmed",
                module_scope=["psd", "erp", "qc"],
        ...
        formal_owner_b_plan = data_preparation_service.save_plan(
            prep_model.DataPreparationPlanCreate(
                project_id="proj_formal_task_guard_owner_b",
                input_file_id="eeg_formal_task_guard_owner_b",
                owner_user_id="owner-b",
        ...
        before_teaching_formal_attempt = {name: _payload_count(state_store, name) for name in formal_state_registries}
        _expect_http(
            "protected teaching file cannot create a formal analysis task",
            422,
        ...
        after_teaching_formal_attempt = {name: _payload_count(state_store, name) for name in formal_state_registries}
        if after_teaching_formal_attempt != before_teaching_formal_attempt:
            raise AssertionError(
                "rejected teaching formal task should not create formal state: "
                f"{before_teaching_formal_attempt} -> {after_teaching_formal_attempt}"
            )

        _expect_http(
            "task creation cannot use another user's data-preparation plan on a formal file",
            403,
            lambda: task_service.create_task(
                task_model.AnalysisTaskCreate(
                    project_id="proj_formal_task_guard_owner_b",
                    ...
                    created_by="owner-b",
                    parameters_json={
                        "data_preparation_plan_id": formal_owner_a_plan.id,
                        "data_preparation_revision": formal_owner_a_plan.revision,
                    },
                ),
                requesting_user_id="owner-b",
            ),
        ...
        first = task_service.create_task(
            task_model.AnalysisTaskCreate(
                ...
                parameters_json={
                    "z": 1,
                    "a": 2,
                    "data_preparation_plan_id": formal_owner_b_plan.id,
                    "data_preparation_revision": formal_owner_b_plan.revision,
                },
            ),
            requesting_user_id="owner-b",
        )
