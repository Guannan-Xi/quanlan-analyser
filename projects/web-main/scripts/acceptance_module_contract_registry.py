...
def main() -> int:
    ...
    if "resting_psd" in templates_by_id and "band_power" in templates_by_id:
        psd_template = templates_by_id["resting_psd"]
        band_power_template = templates_by_id["band_power"]
        if psd_template.get("module") != "psd":
            failures.append(f"psd_module_mismatch:{psd_template.get('module')}")
        if band_power_template.get("module") != "band_power":
            failures.append(f"band_power_module_mismatch:{band_power_template.get('module')}")
        psd_outputs = set(psd_template.get("outputs") or [])
        band_power_outputs = set(band_power_template.get("outputs") or [])
        if {"tables/band_power.csv", "tables/channel_band_power.csv"} & psd_outputs:
            failures.append(f"psd_outputs_include_band_power:{sorted(psd_outputs)}")
        if not {"tables/band_power.csv", "tables/channel_band_power.csv"}.issubset(band_power_outputs):
            failures.append(f"band_power_outputs_missing_tables:{sorted(band_power_outputs)}")
    ...
    pac_v2 = templates_by_id["pac_cfc_v2"]
    if pac_v2.get("module") != "pac_v2":
        failures.append(f"pac_v2_module_mismatch:{pac_v2.get('module')}")
    if pac_v2["module_contract"].get("lifecycle_state") != "beta":
        failures.append(f"pac_v2_lifecycle_mismatch:{pac_v2['module_contract'].get('lifecycle_state')}")
    if str(pac_v2.get("production_status", "")).lower().find("stable") >= 0:
        failures.append(f"pac_v2_status_must_not_be_stable:{pac_v2.get('production_status')}")
    if pac_v2.get("enabled") is not True:
        failures.append("pac_v2_runner_not_enabled")
