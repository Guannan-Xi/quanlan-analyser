...
def render_checklist(preflight: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- `allowed_methods` means the 8 formal analysis methods only: `psd`, `erp`, `tfr`, `multitaper_psd`, `multitaper_tfr`, `reference_csd`, `pac`, `connectivity`",
            "- QC is a data-preparation dependency, not a formal analysis method. Keep `data_preparation_required: true` and `data_preparation_dependency: \"qc\"` instead of adding `qc` to `allowed_methods`.",
            "",
            "## Safety Rule",
            "",
