...
def main() -> int:
    ...
    table_dictionary = json.loads(Path(runner_paths["table_dictionary"]).read_text(encoding="utf-8"))
    scope_contract = json.loads(Path(runner_paths["scope_contract"]).read_text(encoding="utf-8"))
    ...
    if table_dictionary.get("module") != "pac_cfc":
        failures.append(f"table_dictionary_module_mismatch:{table_dictionary.get('module')}")
    if not isinstance(table_dictionary.get("tables"), dict) or "tables/pac_comodulogram_long.csv" not in table_dictionary.get("tables", {}):
        failures.append("table_dictionary_missing_pac_table")
    ...
    if "scope" in scope_contract or "forbidden" in scope_contract or "boundary" in scope_contract:
        failures.append("scope_contract_uses_legacy_fields")
