...
## 2026-07-22 - Merge Spike into the unified QLanalyser application

Goal:

- Replace the standalone `projects\spike-analysis` initialization project with
  an internal Spike module in `projects\web-main`.

Changes:

- Updated workspace and `web-main` product/architecture rules.
- Added `docs\product\spike_analysis_product_and_architecture.md`.
- Added the initial `eeg_core\spike\` domain package boundary.
- No second FastAPI service, frontend, login, database, or task system was
  introduced.

Validation:

- Documentation and package syntax checks are run after the old sibling
  directory is removed.

## 2026-07-22 - Revert: split Spike back out of web-main

Goal:

- Reverse the same-day merge decision above; Spike sorting and analysis moves
  back to an independent top-level pre-research project.

Changes:

- Moved `eeg_core/spike/` content (README, `__init__.py`) to
  `projects/spike-analysis/`; removed `eeg_core/spike/` from `web-main`.
- Rewrote `projects/spike-analysis/README.md` and `__init__.py` to describe it
  as an independent pre-research module following the same
  Lab -> validated -> `eeg_core` lifecycle as `qeeg-64ch-research`, not a
  full-package merge target.
- Added reversal notes to `docs/DECISIONS.md`, `docs/PROJECT_STATUS.md`,
  `docs/PROJECT_STATUS_CURRENT.md`, and `AGENTS.md` (Spike Module Rules moved
  to workspace root docs). Kept
  `docs/product/spike_analysis_product_and_architecture.md` as historical
  background only.

Validation:

- Confirmed no other code under `backend/`, `worker/`, `frontend/` referenced
