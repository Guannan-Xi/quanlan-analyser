...
- 2026-07-22 - Spike 分析统一项目归属。

  Current status:

  - Spike sorting 与分析已定义为 `web-main` 内部模块，不再作为平级应用维护。
  - 产品与架构决策记录在 `docs\product\spike_analysis_product_and_architecture.md`。
  - 已建立领域包 `eeg_core\spike\`；尚未添加未经真实样本验证的 API、sorter 或设备格式实现。
  - 旧的 `projects\spike-analysis` 初始化项目已从本次工作区移除。

- 2026-07-22 - Spike 分析撤回并入，重新拆回独立预研模块。

  Current status:

  - 同一天内撤回上一条"并入 web-main"的决策：Spike sorting 与分析重新拆回
    独立顶层项目 `projects\spike-analysis`，理由与 `qeeg-64ch-research` 一致——
    预研阶段先独立验证，不直接绑定 FastAPI/数据库/任务队列。
  - `eeg_core\spike\` 领域包已从 `web-main` 移除；`web-main` 不再包含 Spike 代码。
  - `docs\product\spike_analysis_product_and_architecture.md` 作为历史决策背景保留，
