...
from backend.services import account_service, customer_delivery_service, data_source_policy, storage_service, task_service
...
def _require_project_mutate(project_id: str, current: AccountRead) -> ProjectRead:
    ...
    if data_source_policy.is_protected_teaching_project(project):
...
def list_projects(current: AccountRead = Depends(account_service.require_current_account)) -> list[ProjectRead]:
    ...
    return [p for p in all_projects if getattr(p, "owner_user_id", None) == current.id and not data_source_policy.is_protected_teaching_project(p)]
...
def get_project(project_id: str, current: AccountRead = Depends(account_service.require_current_account)) -> ProjectRead:
    ...
    if current.role != "admin" and (data_source_policy.is_protected_teaching_project(project) or getattr(project, "owner_user_id", None) != current.id):
