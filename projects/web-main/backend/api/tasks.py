...
from backend.models.public_api import ArtifactPublicRead, artifact_to_public
from backend.services import account_service, audit_service, customer_delivery_service, data_source_policy, storage_service, task_service

router = APIRouter()

_DERIVATIVES_ROOT = (Path(__file__).resolve().parents[2] / "data" / "derivatives").resolve()
_CUSTOMER_ALLOWED_TASK_MODULES = {"qc", "preprocess", "psd", "band_power", "erp"}
    ...
    if current.role != "admin" and payload.module_name not in _CUSTOMER_ALLOWED_TASK_MODULES:
        raise HTTPException(
            ...
            detail={
                ...
                "suggested_action": "Use QC, PSD, Band Power, or ERP first, or ask an operator to enable a lab/internal workflow.",
            },
        )
    if current.role != "admin":
        project = storage_service.get_project(payload.project_id, requesting_user_id=current.id)
        eeg_file = storage_service.get_eeg_file(payload.input_file_id, requesting_user_id=current.id)
        if data_source_policy.is_protected_teaching_project(project) or data_source_policy.is_protected_teaching_file(eeg_file):
            raise HTTPException(
                status_code=422,
                detail={
                    "code": "TEACHING_DATASET_NOT_FORMAL_INPUT",
                    "message": "Teaching/demo data cannot be used to create a formal analysis task.",
                    "suggested_action": "Use the lab demo sandbox endpoint, or upload and authorize a customer EEG file.",
...
@router.get("/tasks/{task_id}/artifacts", response_model=list[ArtifactPublicRead])
def get_task_artifacts(task_id: str, current: AccountRead = Depends(account_service.require_current_account)) -> list[ArtifactPublicRead]:
    # SEC-P0-01 FIX: Verify task ownership before returning artifacts
    task = task_service.get_task(task_id, requesting_user_id=current.id)
    task_service.assert_task_artifacts_deliverable(task)
    return [artifact_to_public(artifact) for artifact in task_service.list_task_artifacts(task_id)]
...
def batch_download_task_artifacts(
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        for artifact in artifacts:
            artifact_path = customer_delivery_service.assert_customer_artifact_file(artifact, task, task_service)
            safe_name = customer_delivery_service.safe_archive_name(artifact.label, artifact_path)
            zip_file.writestr(safe_name, customer_delivery_service.customer_safe_bytes(artifact_path))
