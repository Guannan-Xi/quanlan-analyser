...
from backend.services import audit_service, data_source_policy, quota_service, state_store, storage_service, task_service
...
STABLE_REPORT_SIBLING_MODULES = {"psd", "band_power", "erp"}
STABLE_REPORT_PRIMARY_MODULES = {"psd", "band_power", "erp"}
...
def assert_default_report_primary_module(task) -> None:
    if task.parameters_json.get("lab_preview_run"):
        raise HTTPException(
            ...
            detail={
                ...
                "suggested_action": "Run a formal PSD, Band Power, or ERP analysis from a formal data preparation plan before creating a report.",
            },
        )
    ...
    raise HTTPException(
        ...
        detail={
            ...
            "message": "Default customer report generation is available only for stable PSD, Band Power, or ERP analyses.",
            "module_name": task.module_name,
            "stable_primary_modules": sorted(STABLE_REPORT_PRIMARY_MODULES),
            "suggested_action": "Use PSD, Band Power, or ERP for the default report package; advanced or epilepsy modules require a separate lab/internal workflow.",
        },
    )
...
def assert_report_source_ready(task, reviewer_user_id: str | None = None) -> None:
    ...
    project = storage_service.get_project(task.project_id)
    eeg_file = storage_service.get_eeg_file(task.input_file_id)
...
def _redact_customer_payload(value):
    if isinstance(value, dict):
        return {
            key: _redact_customer_payload(item)
...
def _redact_customer_text(text: str) -> str:
    ...
    redacted = PATH_TOKEN_RE.sub(redact_token, text)
