...
def run_demo_task(module: str, parameters: dict | None = None) -> AnalysisTaskRead:
    ...
    if module not in WORKFLOW_BY_MODULE:
        raise ValueError("Demo supports qc, psd, band_power, erp, tfr, epilepsy, pac, pac_v2, reference_csd, multitaper_psd_tfr, and connectivity")
    if module == "epilepsy_ml":
    ...
    task_parameters.update(
        {
            ...
            "data_source_type": "teaching_demo",
            "lab_preview_boundary": "Teaching/demo sandbox output only; not a formal customer delivery source.",
        }
    )
