...
def enrich_workflow_templates(templates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for template in templates:
        ...
        if contract_id:
            ...
            if record.get("id") == "band_power":
                record["module_contract"] = _band_power_contract_summary(record["module_contract"])
            if record.get("id") == "pac_cfc_v2":
                record["module_contract"] = _pac_v2_contract_summary(record["module_contract"])
        enriched.append(record)
    return enriched


def _band_power_contract_summary(contract: dict[str, Any]) -> dict[str, Any]:
    """Specialize the legacy PSD/bandpower KB contract for the independent Band Power workflow."""
    summary = dict(contract or {})
    summary.update(
        {
            "module_id": "band_power",
            "display_name": "Band Power",
            "task_module": "band_power",
            "workflow_id": "band_power",
            "runner": "eeg_core.analysis.band_power.run_band_power",
            "output_schema_keys": sorted(set(summary.get("output_schema_keys") or []) | {"tables", "figures", "reproducibility_files", "result_json"}),
        }
    )
    return summary


def _pac_v2_contract_summary(contract: dict[str, Any]) -> dict[str, Any]:
    """Specialize the PAC beta KB contract for the PAC V2 lab-only workflow."""
    summary = dict(contract or {})
    summary.update(
        {
            "module_id": "pac_cfc_v2",
            "display_name": "PAC V2 / lab-only coupling metrics",
            "task_module": "pac_v2",
            "workflow_id": "pac_cfc_v2",
            "runner": "eeg_core.analysis.pac_v2.run_pac_v2",
