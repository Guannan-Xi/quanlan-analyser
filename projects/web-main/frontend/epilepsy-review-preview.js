(function () {
  ...
  const EMBEDDED_REVIEW_MODE = new URLSearchParams(window.location.search).get("embed") === "1";
  const REVIEWER_ID = "本地研究复核人";
  const STATUS_LABEL = {
    unreviewed: "未复核",
    kept: "纳入草稿候选",
    excluded: "不纳入草稿",
  ...
  const records = [
    {
      ...
      candidates: [
        ...
        makeCandidate("HE105-E005", 11 * 3600 + 2 * 60 + 28.4, 11 * 3600 + 2 * 60 + 32.2, "low", "rhythmic", ["EEG1", "EEG2"], 0.61, "边界节律片段"),
  ...
  const state = {
    ...
    backendReviewSessionFingerprint: "",
    resizeMessageTimer: 0,
  };

  const dom = {};
  const perfLedger = window.__qlaEpilepsyReviewPerf = createPerfLedger();
  ...
  function makeCandidate(id, startSec, endSec, priority, aiType, channels, previewRankScore, qcHint) {
    return {
      ...
      score_note: "复核优先级排序值不是候选为真实事件的概率、检测置信度或确定性结论。",
      qc_hint: qcHint,
  ...
  function createPerfLedger() {
    return {
      schema_version: "qlanalyser.epilepsy.review_perf.v1",
      created_at: new Date().toISOString(),
      draw_samples_ms: [],
      drag_frame_samples_ms: [],
      waveform_requests: 0,
      cache_hits: 0,
      aborts: 0,
      stale_responses: 0,
  ...
  function pushPerfSample(key, value) {
    const target = perfLedger[key];
    if (!Array.isArray(target) || !Number.isFinite(value)) return;
    target.push(Math.round(value * 100) / 100);
    if (target.length > MAX_PERF_SAMPLES) target.splice(0, target.length - MAX_PERF_SAMPLES);
  }

  function markPerfCache(cacheKey, status) {
    if (!cacheKey) return;
    perfLedger.cache[cacheKey] = { status, at: Date.now() };
  }

  function init() {
    cacheDom();
    document.body.classList.toggle("is-embedded-review", EMBEDDED_REVIEW_MODE);
    ...
    syncIcons();
    notifyParent("qla:epilepsy-review-ready");
  }
    ...
    try {
      ...
      const candidates = payload.reviewed_events.map((event, index) => {
        ...
        return {
          ...
          evidence_window: event.evidence_window || null,
      ...
      state.backendReviewSessionFingerprint = state.backendReviewSession?.session_id ? reviewPayloadFingerprint(payload) : "";
    } catch (error) {
  ...
  function normalizeReviewMetadata(payload, visibleCandidateCount) {
    ...
    return {
      ...
      denominator_note: preserved.denominator_note || (hasFullCandidateSet
        ? "复核记录包含当前已载入的预览候选包；这不是全记录完整检测器的负荷估计。"
  ...
  function renderAll() {
    ...
    updateHandles();
    syncIcons();
    scheduleParentResizeMessage();
  }

  function notifyParent(type, detail = {}) {
    if (!EMBEDDED_REVIEW_MODE || window.parent === window) return;
    window.parent.postMessage({
      schema_version: "qlanalyser.epilepsy.embedded_review_message.v1",
      type,
      ...detail,
    }, window.location.origin);
  }

  function scheduleParentResizeMessage() {
    if (!EMBEDDED_REVIEW_MODE || state.resizeMessageTimer) return;
    state.resizeMessageTimer = window.setTimeout(() => {
      state.resizeMessageTimer = 0;
      const height = Math.max(
        document.body.scrollHeight,
        document.documentElement.scrollHeight,
        document.querySelector("[data-testid='epilepsy-review-preview']")?.scrollHeight || 0,
      );
      notifyParent("qla:epilepsy-review-resize", { height });
    }, 80);
  }
  ...
  function renderSummary() {
    ...
    dom.flowStateText.textContent = reviewed === 0
      ? "请先为当前候选标记“纳入草稿”“存疑”或“不纳入”；未复核的算法候选不能直接进入复核草稿。"
      : remaining
  ...
  function renderReviewPanel() {
    ...
    dom.applyNextBtn.querySelector("span").textContent = review.status === "unreviewed" ? "跳过并下一个" : "保存并进入下一候选";
    dom.applyNextBtn.title = review.status === "unreviewed"
      ? "当前候选还没有判定；点击只会跳到下一个候选。"
      : "保存当前字段并进入下一个候选。";
  }

  function renderReviewTimingFields() {
    const candidate = selectedCandidate();
    if (!candidate) return;
    const review = candidate.review;
    const start = review.adjusted_start_sec ?? candidate.start_sec;
  ...
  function markStatus(status) {
    ...
    withHistory("event_review", () => {
      ...
      candidate.review.reviewer = candidate.review.reviewer || REVIEWER_ID;
  ...
  function syncFormToCandidate() {
    ...
    withHistory("review_fields", () => {
      ...
      if (candidate.review.status !== "unreviewed") candidate.review.reviewer = candidate.review.reviewer || REVIEWER_ID;
    });
  ...
  function drawWaveform() {
    ...
    if (canFetchRealWaveform(record)) {
      drawWaveformLoadingState(ctx, layout, realWindow?.status);
      drawEventLabel(ctx, layout, candidate, sourceLabel);
      return;
    }
    ...
    drawEventLabel(ctx, layout, candidate, sourceLabel);
      ...
      pushPerfSample("draw_samples_ms", performance.now() - drawStartedAt);
    }
  }

  function drawWaveformLoadingState(ctx, layout, status) {
    const message = status === "failed"
      ? "原始 EDF 窗口读取失败，未使用示意波形"
      : "正在读取原始 EDF 窗口，暂不显示示意波形";
    ctx.fillStyle = "#eef6f5";
    ctx.fillRect(layout.left, layout.top, layout.plotW, layout.plotH);
    ctx.strokeStyle = "#b8d6d2";
    ctx.lineWidth = 1;
    ctx.strokeRect(layout.left, layout.top, layout.plotW, layout.plotH);
    ctx.fillStyle = "#31514d";
    ctx.font = "700 15px Inter, Microsoft YaHei, sans-serif";
    ctx.textAlign = "center";
    ctx.fillText(message, layout.left + layout.plotW / 2, layout.top + layout.plotH / 2);
    ctx.font = "12px Inter, Microsoft YaHei, sans-serif";
    ctx.fillStyle = "#68736b";
    ctx.fillText("真实数据返回前不绘制合成波形，避免误读为已加载证据。", layout.left + layout.plotW / 2, layout.top + layout.plotH / 2 + 24);
    ctx.textAlign = "left";
  }
  ...
  function drawRealWaveform(ctx, layout, candidate, payload) {
    ...
    channels.forEach((channel, index) => {
      ...
      const times = numericWaveformArray(channel.times_sec);
      const values = numericWaveformArray(channel.values);
      const yCenter = layout.top + rowH * (index + 0.5);
      ctx.strokeStyle = "#eef2ea";
      ctx.lineWidth = 1;
      ctx.beginPath();
      ...
      const scale = Number(channel.robust_scale) || robustScale(values);
      ...
      for (let pointIndex = 0; pointIndex < values.length; pointIndex += 1) {
        ...
        if (!Number.isFinite(value) || !Number.isFinite(times[pointIndex])) continue;
        const x = xFromTime(times[pointIndex], layout);
        if (x < layout.left - 2 || x > layout.rightX + 2) continue;
  ...
  function numericWaveformArray(values) {
    if (ArrayBuffer.isView(values)) return values;
    return Array.isArray(values) ? values : [];
  }

  function normalizeWaveformPayload(payload) {
    const channels = Array.isArray(payload?.channels) ? payload.channels : [];
    let normalizedChannels = 0;
    const normalized = {
      ...payload,
      channels: channels.map((channel, index) => {
        const times = toFloat32Array(channel.times_sec);
        const values = toFloat32Array(channel.values);
        normalizedChannels += 1;
        return {
          ...channel,
          name: channel.name || `CH${index + 1}`,
          times_sec: times,
          values,
          robust_scale: robustScale(values),
          normalized: true,
        };
      }),
    };
    perfLedger.normalized_channels += normalizedChannels;
    return normalized;
  }

  function toFloat32Array(values) {
    if (values instanceof Float32Array) return values;
    if (ArrayBuffer.isView(values)) return Float32Array.from(values);
    if (!Array.isArray(values)) return new Float32Array();
    return Float32Array.from(values, (value) => Number(value));
  }
  ...
  function updateHandles() {
    ...
    dom.eventBand.setAttribute("aria-label", `拖动候选窗口整体平移，当前时长 ${round1(end - start)} 秒`);
    dom.eventBand.title = `拖动候选窗口整体平移，保持时长 ${round1(end - start)} 秒`;
  ...
  function waveformWindowForCurrentView(record, candidate) {
    ...
    if (cached) {
      cached.at = Date.now();
      if (cached.status === "ready") perfLedger.cache_hits += 1;
      return cached;
    }
  ...
  async function loadBackendWaveformWindow(record, cacheKey) {
    abortPendingWaveformRequest();
    const requestSeq = state.waveformRequestSeq + 1;
    const controller = new AbortController();
    state.waveformRequestSeq = requestSeq;
    state.waveformAbortController = controller;
    state.waveformCache[cacheKey] = { status: "loading", at: Date.now(), requestSeq };
    perfLedger.waveform_requests += 1;
    markPerfCache(cacheKey, "loading");
    pruneWaveformCache();
    try {
      ...
      if (state.waveformRequestSeq !== requestSeq || currentRecord()?.id !== record.id) {
        perfLedger.stale_responses += 1;
        return;
      }
      state.waveformCache[cacheKey] = { status: "ready", payload: normalizeWaveformPayload(payload), at: Date.now() };
      perfLedger.ready_responses += 1;
      markPerfCache(cacheKey, "ready");
      pruneWaveformCache();
      drawWaveform();
    } catch (error) {
      ...
      perfLedger.failed_responses += 1;
      markPerfCache(cacheKey, "failed");
      pruneWaveformCache();
      addAudit("backend_waveform_failed", "原始 EDF 窗口读取失败", selectedCandidate()?.id || "");
      drawWaveform();
    } finally {
      if (state.waveformRequestSeq === requestSeq) state.waveformAbortController = null;
    }
  }
  ...
  function pruneWaveformCache() {
    ...
    entries
      ...
      .forEach(([key]) => {
        ...
        delete perfLedger.cache[key];
      });
  }

  function abortPendingWaveformRequest() {
    const controller = state.waveformAbortController;
  ...
  function scheduleDragRender() {
    ...
    state.dragRaf = requestAnimationFrame(() => {
      ...
      state.dragRaf = 0;
      renderReviewTimingFields();
      drawWaveform();
      drawTimeline();
      updateHandles();
      pushPerfSample("drag_frame_samples_ms", performance.now() - frameStartedAt);
    });
  }
  ...
  function evidenceWindowForReview(candidate, start, end) {
    if (!candidate.evidence_window) return null;
    return {
      ...candidate.evidence_window,
      start_sec: round1(start),
      stop_sec: round1(end),
      duration_sec: round1(end - start),
      source: candidate.evidence_window.source || "real_edf_reviewed_window",
    };
  }
    ...
    return {
      ...
      event_reviews: Object.fromEntries(
        record.candidates.map((candidate) => [
          candidate.id,
          {
            ...
            reviewer: candidate.review.reviewer || "复核人待记录",
      ...
      reviewed_events: record.candidates.map((candidate) => {
        ...
        return {
          ...
          evidence_window: evidenceWindowForReview(candidate, start, end),
          ...
          reviewer: backendStatus === "unreviewed" ? (candidate.review.reviewer || "") : (candidate.review.reviewer || REVIEWER_ID),
          reviewed_at: candidate.review.reviewed_at || "",
        };
      }),
      actions: state.audit,
  ...
  function reviewMetadata(record) {
    ...
    return {
      ...
      denominator_note: "当前复核记录包含已载入的预览候选包；这不是完整检测器的全记录负荷估计。",
    };
  }
  ...
  async function copyJson() {
    try {
      const saved = await saveBackendReviewSession();
      if (!saved?.session_id) {
        showToast("当前记录尚未形成后端可追溯复核会话，不能复制交付用复核包。");
        return;
      }
      const exportPackage = await apiFetch(`/lab/epilepsy-full-flow/review-sessions/${encodeURIComponent(saved.session_id)}/exports`, {
        method: "POST",
      });
      const text = JSON.stringify(exportPackage, null, 2);
      await navigator.clipboard.writeText(text);
      showToast("后端可追溯复核包已复制");
    } catch (error) {
      console.warn("Backend review export copy failed", error);
      showToast("未能复制后端可追溯复核包，请检查服务后重试。");
    }
  }
  ...
  async function exportJson() {
    try {
      const saved = await saveBackendReviewSession();
      if (!saved?.session_id) {
        showToast("当前记录尚未形成后端可追溯复核会话，不能下载交付用复核包。");
        return;
      }
      const exportPackage = await apiFetch(`/lab/epilepsy-full-flow/review-sessions/${encodeURIComponent(saved.session_id)}/exports`, {
        method: "POST",
      ...
      const text = JSON.stringify(exportPackage, null, 2);
      saveBlob(text, `${currentRecord().id}_复核草稿_${saved.session_id}.json`, "application/json;charset=utf-8");
      showToast("后端可追溯复核包已下载");
    } catch (error) {
      console.warn("Backend review export failed", error);
      showToast("复核包未能从后端生成，已阻断本地下载回退。请检查服务后重试。");
    }
  }
  ...
  async function openReportPreview() {
    ...
    if (!persistLatestReviewPayload()) {
      showToast("无法临时保存复核草稿，不能生成可追溯报告预览。请刷新后重试。");
      return;
    }
    try {
      await saveBackendReviewSession();
    } catch (error) {
      showToast("复核记录未保存到后端，不能生成可追溯草稿。请检查服务后重试。");
      console.warn("Backend review session save skipped before report", error);
      return;
    }
    if (EMBEDDED_REVIEW_MODE) {
      notifyParent("qla:epilepsy-review-open-report", {
        backend_review_session_id: state.backendReviewSession?.session_id || "",
      });
      showToast("复核记录已同步到全流程页，可继续查看草稿。");
      return;
    }
  ...
  function persistLatestReviewPayload() {
    try {
      ...
      notifyParent("qla:epilepsy-review-updated", {
        reviewed_count: payload.summary?.reviewed || 0,
        backend_review_session_id: payload.backend_review_session?.session_id || "",
      });
      return true;
    } catch (error) {
  ...
  async function returnToFullFlow() {
    if (!persistLatestReviewPayload()) {
      showToast("无法临时保存复核草稿，不能完成可追溯回传。请刷新后重试。");
      return;
    }
    try {
      await saveBackendReviewSession();
    ...
    if (EMBEDDED_REVIEW_MODE) {
      notifyParent("qla:epilepsy-review-return-flow", {
        backend_review_session_id: state.backendReviewSession?.session_id || "",
      });
      showToast("复核状态已同步到全流程页。");
      return;
    }
  ...
  async function saveBackendReviewSession() {
    ...
    const payload = reviewPayload();
    const fingerprint = reviewPayloadFingerprint(payload);
    if (state.backendReviewSession?.session_id && state.backendReviewSessionFingerprint === fingerprint) {
      persistLatestReviewPayload();
      return state.backendReviewSession;
    }
    ...
    state.backendReviewSessionFingerprint = fingerprint;
    addAudit("backend_review_session_saved", "复核记录已同步", saved.session_id || "");
    persistLatestReviewPayload();
    return saved;
  }

  function reviewPayloadFingerprint(payload) {
    const events = (payload.reviewed_events || []).map((event) => ({
      event_id: event.event_id || "",
      status: event.status || event.backend_status || "",
      event_type: event.event_type || event.reviewed_type || "",
      start_sec: event.start_sec,
      end_sec: event.end_sec,
      reviewed_start_sec: event.reviewed_start_sec,
      reviewed_end_sec: event.reviewed_end_sec,
      evidence_grade: event.evidence_grade || "",
      artifact_reason: event.artifact_reason || "",
      include_in_report: Boolean(event.include_in_report),
    ...
    return JSON.stringify({
      ...
      context: {
        ...
        input_file_id: context.input_file_id || "",
