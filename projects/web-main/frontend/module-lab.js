...
const LAB_PLAN_MODULE_SCOPE = ["qc", "psd", "band_power", "erp", "epilepsy", "tfr", "pac", "pac_v2", "reference_csd", "multitaper_psd_tfr", "connectivity"];
...
async function loadDemo数据集() {
  try {
    ...
    state.demo数据集 = null;
    const demoButton = document.querySelector("#labDemoModeButton");
    if (demoButton) {
      demoButton.disabled = true;
      demoButton.title = "教学示例数据当前不可用";
    }
...
function updateRunButtonStates() {
  document.querySelectorAll("[data-runner-form]").forEach((form) => {
    ...
    const selectedValue = form.elements.dataset?.value || "";
    const disabled = !selectedValue;
    if (!button) return;
    button.disabled = disabled;
    button.title = disabled ? "请先上传或选择 EEG 数据；教学示例需显式进入示例模式。" : "";
...
function renderFileOptions() {
  document.querySelectorAll("[data-file-select]").forEach((select) => {
    const currentValue = state.selectedFileId || select.value;
    select.innerHTML = [
      `<option value="">未选择 EEG 数据</option>`,
      state.demoMode ? `<option value="__demo__">教学示例 Oddball EEG（仅用于方法演示）</option>` : "",
      ...state.files.map((file) => `<option value="${h(file.id)}">${h(fileOptionText(file))}</option>`),
    ].join("");
    if (state.demoMode && currentValue === "__demo__") {
      select.value = "__demo__";
    } else {
      select.value = state.files.some((file) => file.id === currentValue) ? currentValue : "";
    }
  });
...
async function runModule(moduleId, form, resultBox) {
  ...
  if (selectedFileId && selectedFileId !== "__demo__") {
  ...
  } else if (selectedFileId === "__demo__") {
    ...
    throw new Error("请先上传或选择 EEG 数据；教学示例需显式进入示例模式。");
  }
...
function bindRunners() {
  ...
  document.querySelectorAll("[data-runner-form]").forEach((form) => {
    form.addEventListener("submit", async (event) => {
      ...
      if (!form.elements.dataset?.value) {
        renderDataSourceStatus("请先上传或选择 EEG 数据；教学示例需显式进入示例模式。", "info");
        updateRunButtonStates();
        return;
      }
  ...
  const demoButton = document.querySelector("#labDemoModeButton");
  if (demoButton && !demoButton.dataset.boundDemoMode) demoButton.addEventListener("click", () => {
    if (!state.demo数据集?.file) {
      renderDataSourceStatus("教学示例数据当前不可用；请上传或选择 EEG 数据。", "info");
      return;
    }
    state.demoMode = true;
    state.selectedFileId = "__demo__";
    renderFileOptions();
    renderDataSourceStatus("已进入教学示例模式。示例产物仅用于方法演示，不能作为正式客户报告来源。", "ok");
  });
  if (demoButton) demoButton.dataset.boundDemoMode = "true";
  if (!document.body.dataset.boundModuleLabFileSelect) {
    document.addEventListener("change", (event) => {
      if (!event.target.matches("[data-file-select]")) return;
      state.demoMode = event.target.value === "__demo__";
    ...
    <section class="panel lab-source-panel">
      ...
      <form class="runner-form source-form" onsubmit="return false;">
        ...
        <div class="source-actions">
          ...
          <button id="labDemoModeButton" class="btn" type="button">${icon("graduation-cap")}使用教学示例</button>
        </div>
      </form>
...
function renderDataSourceStatus(message = "", tone = "info") {
  ...
  if (!box) return;
  const file = selectedCustomerFile();
  const demoFile = state.demoMode ? state.demo鏁版嵁闆?.file : null;
  const sourceFile = file || demoFile;
  const eventsCounts = file ? (file.metadata_json?.events || null) : (demoFile?.metadata_json?.events || null);
  const sourceLabel = file
    ? `当前客户 EEG：${fileOptionText(file)}`
    : demoFile
      ? "当前使用教学示例 Oddball EEG（仅用于方法演示，不可生成正式报告）"
      : "未选择 EEG 数据。请先上传或选择客户 EEG；教学示例需显式进入示例模式。";
  const detailItems = [
    eventsCounts && {
      label: "事件数量",
      value: Object.entries(eventsCounts).map(([key, value]) => `${key}: ${value}`).join(" / "),
    },
    sourceFile?.id && { label: "来源编号", value: sourceFile.id },
  ].filter(Boolean);
  box.className = `demo-status ${tone === "ok" ? "ok" : ""}`;
  box.innerHTML = `
    <strong>${h(message || sourceLabel)}</strong>
    <span>${state.demoMode ? "示例产物仅用于方法学习/沙盒回归，不能作为客户正式报告来源。" : "运行前必须明确数据来源；结果会记录参数、来源和科研边界。"}</span>
    ${detailItems.length ? `<div class="data-source-details">${detailItems.map((item) => `<div class="data-source-detail"><span>${h(item.label)}</span><code>${h(item.value)}</code></div>`).join("")}</div>` : ""}
  `;
}
