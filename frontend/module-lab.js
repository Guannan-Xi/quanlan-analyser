...
const LAB_STAGES = [
  ...
  },
  {
    ...
    body: "按研究问题选择分支：质控、频谱、事件相关反应或高级预研分析",
    evidence: ["QC / PSD / ERP 已启用", "TFR / PAC / Connectivity 预研", "每个分支保留 MNE 对应方法"],
  },
  {
    id: "statistics",
    icon: "sigma",
    title: "统计",
    subtitle: "指标表与条件对比",
    body: "在被试或条件指标上做统计，保留效应量、校正方法和人工复核边界",
...
const REVIEW_ROWS = [
  ["enabled", "先看数据能不能用", "QC 质控", "EEG 文件、通道信息、采样率、查看时间窗", "波形预览、质量摘要、异常提示、质控图、结果包", "正式分析前先做"],
  ["enabled", "看频段功率分布", "PSD 频谱", "静息态或任务态 EEG、频段范围、通道选择、分析时间段", "功率谱图、频段功率表、参数记录、方法说明", "比较组别或条件前先确认"],
  ["enabled", "看事件相关反应", "ERP", "连续 EEG、事件表、刺激编码、基线窗、分段时间窗", "平均波形、条件对比图、峰值指标、drop log、结果文件", "适合 P300 / N400 等任务"],
  ["preview", "看时频变化", "TFR", "事件分段、频率范围、baseline、decimation、内存预算", "时频图、功率变化矩阵、参数记录", "先用于方案确认"],
  ["preview", "看跨频耦合", "PAC", "相位频段、振幅频段、时间窗、替代检验、统计阈值", "耦合热图、显著性说明、方法边界", "需要人工复核"],
  ["preview", "看脑区连接关系", "Connectivity", "分区或通道方案、连接指标、频段、参考方式、体积传导控制", "连接矩阵、网络图、指标表、风险提示", "只作预研展示"],
  ["all", "确认科研交付物", "所有模块", "研究目的、数据类型、参数选择、复核人", "科研级图表、CSV、JSON、方法说明、完整下载包", "下载前核对参数"],
];
...
function renderTopbar(manifest, compact = false) {
  return `<nav class="lab-nav ${compact ? "compact" : ""}">
    <a class="brand" href="./module-lab.html" aria-label="EEG 分析流程">
      <span class="brand-mark">QL</span>
      <span><strong>分析流程</strong><small>免登录预览</small></span>
    </a>
    <div class="quick">
      <a href="./index.html">${icon("home")}正式工作台</a>
      <a href="./research-modules.html">${icon("layout-dashboard")}流程总览</a>
...
function renderOpenDesignDemo(manifest) {
  ...
  return `<section class="demo-band" data-open-design-demo>
    <div class="section-head">
      <p class="eyebrow">EEG workflow</p>
      <h1>按脑电分析流程预览</h1>
      <p>从项目、数据到统计、绘图和下载，先看清每一步的输入输出</p>
    </div>
    <div class="demo-workbench">
      <aside class="stage-rail" aria-label="EEG 分析流程">
      ...
      <article class="demo-canvas">
        <div class="canvas-toolbar">
          <span class="status enabled">免登录预览</span>
        ...
        <div class="module-strip" aria-label="分析分支快速选择">
        ...
        <div class="canvas-actions">
          <a class="btn primary" href="./module-lab.html?module=${h(featured?.slug || "qc")}">${icon("play")}查看 ${h((featured?.slug || "qc").toUpperCase())}</a>
          <a class="btn" href="#review-plan">${icon("list-checks")}查看分析分支</a>
        </div>
      </article>
      <aside class="handoff-panel">
        <h2>流程边界</h2>
        <dl>
          <div><dt>正式数据</dt><dd>登录后在工作台管理</dd></div>
          <div><dt>预览区</dt><dd>用于流程和样例输出确认</dd></div>
          <div><dt>交付物</dt><dd>图表、表格、方法和复现记录</dd></div>
        </dl>
      </aside>
    </div>
  </section>`;
  ...
  return `<section class="review-band" id="review-plan" data-review-matrix>
    <div class="section-head row-head">
      <div>
        ...
        <h2>分析分支怎么选</h2>
        <p>按研究目的匹配模块，优先确认数据、参数、输出和边界</p>
      </div>
      <div class="segmented" role="group" aria-label="模块范围过滤">
    ...
    <div class="table-wrap">
      <table class="review-table">
        <thead><tr><th>研究目的</th><th>优先模块</th><th>准备材料</th><th>输出</th><th>注意</th></tr></thead>
        <tbody>
          ${REVIEW_ROWS.map(([scope, question, moduleName, inputs, outputs, gate]) => `<tr data-review-scope="${scope}"><td><strong>${h(question)}</strong><small>${scope === "all" ? "全流程" : scope === "enabled" ? "已启用" : "预研"}</small></td><td>${h(moduleName)}</td><td>${h(inputs)}</td><td>${h(outputs)}</td><td><span class="gate">${h(gate)}</span></td></tr>`).join("")}
        </tbody>
      </table>
    </div>
  </section>`;
...
function renderModuleGrid(manifest) {
  return `<section class="module-band">
    <div class="section-head">
      <p class="eyebrow">Analysis branches</p>
      <h2>分析分支</h2>
      <p>QC / PSD / ERP 可执行，TFR / PAC / Connectivity 用于方案预研</p>
    </div>
    <div class="module-grid">
      ${modules(manifest).map((module) => `<article class="module-card">
        ...
        <div class="body">
          ...
          <div class="actions">
            <a class="btn primary" href="./module-lab.html?module=${h(module.slug)}">${icon("external-link")}查看</a>
  ...
  <main class="lab-wrap detail-shell">
    ...
    <div class="content">
      <section class="module-hero">
        <span class="status ${statusClass(module)}">${h(statusLabel(module))}</span>
        <p class="eyebrow">${h(module.slug.toUpperCase())} analysis branch</p>
      ...
      <section class="panel grid-2" id="inputs">
        <div><h2>数据输入</h2>${list(module.inputs)}</div>
        <div id="controls"><h2>参数与预处理</h2>${list(module.controls)}</div>
      </section>
      <section class="panel grid-2" id="mne">
        <div><h2>MNE 方法</h2>${list(module.mneObjects)}</div>
        <div id="outputs"><h2>指标输出</h2>${list(module.outputs)}</div>
      </section>
      <section class="panel" id="figures"><h2>绘图指标输出</h2><div class="figure-grid">${(module.figures || []).map((fig) => `<figure class="figure"><img src="${asset(fig.src)}" alt="${h(fig.alt || fig.label)}" /><figcaption>${h(fig.label)}</figcaption></figure>`).join("")}</div></section>
      <section class="panel" id="artifacts"><h2>下载文件</h2><div class="artifact-grid">${artifactCards(module)}</div></section>
      <section class="panel" id="tests"><h2>复核清单</h2><div class="table-wrap"><table class="test-matrix"><thead><tr><th>检查项</th><th>验收规则</th></tr></thead><tbody>${testRows(slug)}</tbody></table></div></section>
      <section class="panel callout" id="risks"><h2>边界与风险</h2>${list(module.risks, "risk")}<p><strong>模块状态：</strong>${h(handoff[slug])}</p><p><strong>共享边界：</strong>${h(manifest.researchGuardrail)}</p></section>
    </div>
  </main>`;
}
